 ...
 */
public class fr_providerprofile1 extends Fragment implements AsyncResponse {
    private int PLACE_PICKER_REQUEST = 1;
    private static final int PLACES_RETURN_RESULT = 2;
    private EditText txtfullname, txtmobile1, txtmobile2, txtemail, address, txtyear, txtteam, addressloc, txtcontact, txtradius;
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container,
        ...
        btnnext.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                ...
                Fragment myFragment = new fr_providerprofile2();
                getActivity().getSupportFragmentManager().beginTransaction().replace(R.id.fragment_0, myFragment).addToBackStack(null).commitAllowingStateLoss();
            }
        });
        return v;
...
//                String address = String.format("%s", data.getStringExtra("keyLocation"));
    ...
    private void fetchData() {
        ...
        try {
            ...
            aasyncTask.delegate = fr_providerprofile1.this;
    ...
    public void processFinish(String output, String handle) throws HMOwnException, JSONException {
        ...
        JSONArray json_array_services = myjson.getJSONArray("servicelist");
        JSONArray json_array_documents = myjson.getJSONArray("documentlist");
        //Page 1 set
        providerinfo.setFullName(myjson.getString("companyname"));
        providerinfo.setMobile1(myjson.getString("mobile1"));
        providerinfo.setMobile2(myjson.getString("mobile2"));
        providerinfo.setEmail(myjson.getString("emailid"));
        providerinfo.setYear(myjson.getString("yearofformation"));
        providerinfo.setTeamSize(myjson.getString("teamsize"));
        providerinfo.setAddress(myjson.getString("address"));
        providerinfo.setLatLon(myjson.getString("geolocation"));
        providerinfo.setQualification(myjson.getString("qualification"));
        providerinfo.setEntity(myjson.getString("individualorcompany"));
        providerinfo.setGender(myjson.getString("gender"));
        providerinfo.setContactName(myjson.getString("contact"));
        providerinfo.setProfileImage(myjson.getString("profilepicture"));
        providerinfo.setRadius(myjson.getString("radius"));
        providerinfo.setGSTNumber(myjson.getString("gstnumber"));
        //Page 2 set
        providerinfo.clearServiceItem();
        for (int i = 0; i < json_array_services.length(); i++) {
            objects = json_array_services.getJSONObject(i);
            providerinfo.addServiceItem(objects.getString("servicecode"));
            Log.i("Debugging", "Service List Output  Output  " + objects.getString("servicecode"));
            Log.i("Debugging", "Retrieve Service Item  " + providerinfo.getServiceItem(objects.getString("servicecode")));
        }
        Log.i("Debugging", "Retrieve All Service Item  " + providerinfo.getAllServiceItems().toString());

        //Page 3 set
        for (int i = 0; i < json_array_documents.length(); i++) {
            objects = json_array_documents.getJSONObject(i);
            if (objects.getString("documentcode").equals("A")) {
                providerinfo.setAadhar(objects.getString("documentref"));
                providerinfo.setAadharImage(objects.getString("documentimage"));
            }
            if (objects.getString("documentcode").equals("T")) {
                providerinfo.setTrade(objects.getString("documentref"));
                providerinfo.setTradeImage(objects.getString("documentimage"));
            }
            if (objects.getString("documentcode").equals("D")) {
                providerinfo.setDL(objects.getString("documentref"));
                providerinfo.setDLImage(objects.getString("documentimage"));
            }
        }
        //Page 4 Set
        providerinfo.setAccount(myjson.getString("bankaccount"));
        providerinfo.setIFSC(myjson.getString("ifsccode"));
        providerinfo.setReferral(myjson.getString("referral"));
        ...
        if (providerinfo.getFullName().length() > 0) {
            ...
            for (int i = 0; i < spnqualify.getCount(); i++) {
                Log.i("Debugging", "Spinner Qualification " + providerinfo.getQualification() + "    " + spnqualify.getItemAtPosition(i).toString().substring(0, 1));
                if ((spnqualify.getItemAtPosition(i).toString().substring(0, 1)).equals(providerinfo.getQualification())) {
